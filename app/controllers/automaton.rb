...
class Automaton
  ...
  def to_yaml
    ...
    h.to_yaml.lines[1..-1].join # Remove first line (---\n)
  end
  ...
  def self.generate_random
    create_new_from_goal(`goal generate -r -A classical`)
  end   
  ...
  # Create a file in standard tmp directory (/tmp), write a string to it
  # and return the file name.
  def write_tmp_file(string)
    production = "mktemp"
    development = "mktemp -t XXXXXXXXX"
    file = File.open(`#{development}`, "w")   # Default location for mktemp is /tmp
    file.write(string)
    file.close
    file.path
  end
  
  # Generate SVG image of automaton with Graphviz and return SVG string.
  def draw
    ...
    # From http://rdoc.info/github/glejeune/Ruby-Graphviz/GraphViz:output
